...
function ebscoHostSearchRepositoro(form, buff) {
    var ebscohostsearchtext = chkObject(form.ebscohostsearchtext,'');
    var ebscohostkeywords = chkObject(form.ebscohostkeywords,'');
    var ebscohostsearchsrc = chkObject(form.ebscohostsearchsrc,'');
    var ebscohostsearchmode = chkObject(form.ebscohostsearchmode,'\+AND\+');
    ...
    var selectedIR = " AND LN ir00238a";
    ebscohostsearchtext = encodeURI(ebscohostsearchtext);
...
function ebscoHostSearchPortalRevistas(form, buff) {
    ...
    var strAlert="";
    //var selectedIR = selectedRadio();
    var selectedIR = " AND LN ir00353a";
    ebscohostsearchtext = encodeURI(ebscohostsearchtext);
...
function handleCatalogSearch() {
    if(jQuery('#chkCatalogOnly').is(':checked')) {
        jQuery("#cat_locations").show();
    } else {
        jQuery("#cat_locations").hide();
    }
}
...
function ebscoHostSearchGo(form) {
    ...
    var e = document.getElementById("searchFieldSelector");
    // var searchFieldfield = e.options[e.selectedIndex].value;
    var searchFieldfield = e.value;


    if(searchFieldfield=="AU") {
        ebscohosturl+='&bquery=AU+('+ebscohostkeywords+ebscoHostSearchParse(ebscohostsearchtext,ebscohostsearchmode)+')';
    } else if(searchFieldfield=="TI") {
    ...
    if (!_isEmpty(cbCO) && cbCO.checked) {
        ...
        ebscohosturl+='+AND+(';
        jQuery('#common_LB :selected').each(function(i, selected){
            console.log(jQuery(selected));
            if (jQuery(selected)[0].value!="") {
                ebscohosturl+='+OR+'+jQuery(selected)[0].value;
            }
        });
        ebscohosturl+=')';
    }
...
function ebscoHostSearchArticles(form, buff) {
    var ebscohostsearchtext = chkObject(form.ebscohostsearchtext,'');
    var ebscohostkeywords = chkObject(form.ebscohostkeywords,'');
    var ebscohostsearchsrc = chkObject(form.ebscohostsearchsrc,'');
    var ebscohostsearchmode = chkObject(form.ebscohostsearchmode,'\+AND\+');
